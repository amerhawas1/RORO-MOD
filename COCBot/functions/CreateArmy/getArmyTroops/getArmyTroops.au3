...
Func getArmyTroops($bOpenArmyWindow = False, $bCloseArmyWindow = False, $bCheckWindow = False, $bSetLog = True, $bNeedCapture = True)
	...
	If _CheckPixel($aRecievedTroops, $bNeedCapture) Then ; Found the "You have recieved" Message on Screen, wait till its gone.
		If $bSetLog Then SetLog("كشف قلعة عشيرة رسالة حظر الصور القوات. انتظر حتى ذهب", $COLOR_INFO)
		_CaptureRegion2()
	...
	If UBound($aCurrentTroops, 1) >= 1 Then
		For $i = 0 To UBound($aCurrentTroops, 1) - 1 ; Loop through found Troops
			...
			$sTroopName = $g_aiCurrentTroops[$iTroopIndex] >= 2 ? $g_asTroopNames[$iTroopIndex] : $g_asTroopNames[$iTroopIndex] ; Select the right Troop Name, If more than one then use the Plural
			If $bSetLog Then SetLog("   " & $g_aiCurrentTroops[$iTroopIndex] & " " & $sTroopName & " متاح ", $COLOR_SUCCESS) ; Log What Troop is available and How many
		Next
	EndIf
