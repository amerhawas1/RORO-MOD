...
Func CheckStopForWar()
	...
	If $g_iStartTimerToRecheck <> 0 Then
		...
		If $g_sCheckOrStop = "Time to stop" Then SetLog(" يجب أن يكون الوقت للتوقف للحرب الآن. لنلقي نظرة ", $COLOR_INFO)
	EndIf
	...
	If Not $bCurrentWar Then
		...
		SetLog(" سوف يعود للتحقق في 6 ساعات ", $COLOR_INFO)
	...
	Else
		If Not $bInWar Then
			...
			If $g_iTimerToRecheck > 0 Then
				SetLog(" سوف يعود للتحقق بعد انتهاء الحرب الحالية في " & Int($g_iTimerToRecheck / 60) & "h " & Mod($g_iTimerToRecheck, 60) & "m", $COLOR_INFO)
				$g_sCheckOrStop = "Time to re-check"
			Else
		...
		Else
			...
			If $g_iTimerToRecheck > 0 Then
				$g_iStartTimerToRecheck = TimerInit()
				SetLog(" سوف تتوقف استعدادا للحرب في " & Int($g_iTimerToRecheck / 60) & "h " & Mod($g_iTimerToRecheck, 60) & "m", $COLOR_INFO)
			...
			Else
				...
				If $iSleepTime >= 60 Then
					SetLog(" توقف والاستعداد للحرب الآن ", $COLOR_INFO)
					$bStopAction = True
				Else
					SetLog(" حان الوقت للتوقف للحرب. لكن إيقاف نافذة الوقت ضيق للغاية ، فما عليك سوى التخطي والاستمرار ", $COLOR_INFO)
					SetLog(" سوف يعود للتحقق في 6 ساعات ", $COLOR_INFO)
					$g_iStartTimerToRecheck = TimerInit()
					$g_iTimerToRecheck = 6 * 60 ; 360 minutes = 6 hours
					$g_sCheckOrStop = "Time to re-check"
...
Func CheckWarTime(ByRef $aResult) ; return Success + $aResult[3] = [ $iBattleStartTime, $iBattleEndTime, $bInWar] OR Failure
	...
	If IsWarMenu() Then
		...
		If Not StringInStr($sWarDay, "Battle") And Not StringInStr($sWarDay, "Preparation") Then
			SetLog(" عشيرتك ليست في حرب نشطة حتى الآن. ", $COLOR_INFO)
		...
		Else
			...
			If StringInStr($sWarDay, "Preparation") Then
				...
				SetLog(" حرب العشائر هي الآن في التحضير. سوف تبدأ المعركة " & $sTime, $COLOR_INFO)
			ElseIf StringInStr($sWarDay, "Battle") Then
				$aResult[1] = $iConvertedTime ; $iBattleEndTime
				SetLog(" حرب العشائر هي الآن في يوم المعركة. معركة ستنتهي في " & $sTime, $COLOR_INFO)
			EndIf
	...
	Else
		SetLog(" خطأ عند محاولة فتح نافذة الحرب. ", $COLOR_WARNING)
...
Func StopAndPrepareForWar($iSleepTime)
	...
	If $g_bTrainWarTroop Then
		SetLog(" دعونا إزالة جميع القوات الزراعية وتدريب قوات الحرب ", $COLOR_ACTION)
		...
		If Not $g_bUseQuickTrainWar Then
			...
			; Train 2nd set
			SetLog(" دعونا تدريب 2 مجموعة من القوات ونوبات ")
			...
			SetLog(" التحقق من علامة تبويب القوات: " & $TroopCamp[0] & "/" & $TroopCamp[1] * 2)

			$rWhatToTrain = WhatToTrain(False, True)
			TrainUsingWhatToTrain($rWhatToTrain, True)
			If _Sleep(750) Then Return
			Local $SpellCamp = GetOCRCurrent(43, 160)
			SetLog(" التحقق من علامة التبويب الإملائي: " & $SpellCamp[0] & "/" & $SpellCamp[1] * 2)
	...
	If $g_bRequestCCForWar Then
		...
		SetLog(" دعونا نطلب مرة أخرى للحرب ", $COLOR_ACTION)
		RemoveCC()
		RequestCC()
	EndIf

	SetLog(" حان وقت الحرب ، لنأخذ استراحة ", $COLOR_ACTION)
	...
	If ProfileSwitchAccountEnabled() Then
		If GUICtrlRead($g_ahChkAccount[$g_iCurAccount]) = $GUI_CHECKED Then
			...
			ElseIf UBound($aActiveAccount) = 1 Then
				SetLog(" هذا هو الحساب النشط الأخير للتبديل ، أغلق اللعبة على أي حال ")
			EndIf
		EndIf
	EndIf
...
Func RemoveCC()
	...
	If $bHasCCTroopOrSpell Then
		...
		For $i = 0 To 10
			...
			Else
				If $i = 10 Then
					SetLog(" لا يمكن العثور على / التحقق من الزر موافق ", $COLOR_WARNING)
					ClickP($aAway, 2, 0)
		...
		For $i = 0 To 10
			...
			Else
				If $i = 10 Then
					SetLog(" لا يمكن العثور على / التحقق من الزر موافق # 2 في علامةتابعة الجيش ", $COLOR_WARNING)
					ClickP($aAway, 2, 0)
					Return ; Exit function
				EndIf
				If _Sleep(300) Then Return
			EndIf
		Next
		SetLog(" القوات / نوبات إزالتها ", $COLOR_SUCCESS)
		If _Sleep(200) Then Return
	EndIf
	...
	For $i = 0 To $g_iTotalAcc
		...
		If $g_aiStartTimerToRecheck[$i] <> 0 Then
			...
			If $iCheckTimer >= $g_aiTimerToRecheck[$i] * 60 * 1000 And $g_asCheckOrStop[$i] = "Time to stop" Then
				SetLog(" الحساب [" & $i + 1 & "] should stop for war now.", $COLOR_INFO)
